...
export default function WeChatLoginDialog() {
  ...
  useEffect(() => {
    return subscribeLoginPrompt((payload) => {
      ...
      setReason(payload.reason || '请使用微信头像和昵称完成登录后继续。')
      setNickname('')
      setAvatarPath('')
      setVisible(true)
    })
  ...
  const confirm = () => {
    ...
    pendingRef.current = null
    setVisible(false)
    pending?.resolve({ nickname: name, avatarPath })
  }
    ...
    <View className='wechat-login-mask'>
      <View className='wechat-login-panel'>
        ...
        <View className='wechat-login-actions'>
          <Button className='wechat-login-cancel' hoverClass='wechat-login-hover' onClick={cancel}>
            取消
          </Button>
